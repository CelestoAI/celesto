...
Connects the Celesto Python SDK to the React+Vite frontend via REST
endpoints and WebSocket streaming.

Usage:
    uvicorn celesto.dashboard.server:app --host 0.0.0.0 --port 8000
"""
...
from celesto.utils import unsafe_tar_member_kind
...
def _resolve_ui_dist_path() -> Path:
    ...
    # Source checkout layout: <repo>/src/celesto/dashboard/server.py + <repo>/ui/dist
    if repo_ui_dir.is_dir() or (repo_root / ".git").exists():
        return repo_ui_dir / "dist"

    # Installed package fallback: cache downloaded UI under writable Celesto state.
...
def _extract_dashboard_dist(archive_path: Path, extract_dir: Path) -> Path:
    ...
    with tarfile.open(archive_path, "r:gz") as tar:
        # One shared rule — see celesto.utils.unsafe_tar_member_kind.
...
def _get_sdk(app: FastAPI) -> CelestoManager:
    """Get the SDK instance from app.state, raising if not initialized."""
    sdk: CelestoManager | None = getattr(app.state, "sdk", None)
    if sdk is None:
        raise RuntimeError("CelestoManager not initialized.")
    return sdk
...
# --- App ---
app = FastAPI(
    title="Celesto Nebula Dashboard",
...
async def open_vm_desktop(vm_id: str) -> dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=(
                f"Sandbox '{vm_id}' was not found; run 'celesto sandbox list --all' to choose "
                "an existing sandbox."
            ),
        ) from None
    ...
    except (OSError, CelestoError) as exc:
        raise HTTPException(
            status_code=409,
            detail=(
                f"Sandbox '{vm_id}' desktop could not open; run "
                f"'celesto sandbox desktop {vm_id}' to retry."
            ),
        ) from exc
