...
from celesto.host.manager import HostCapability, HostManager
from celesto.host.network import NetworkManager, check_network_prerequisites, resolve_domains_to_ips
...
from celesto.runtime.base import (
...
from celesto.runtime.guest_platforms import get_guest_platform
from celesto.runtime.libkrun import LibkrunRuntimeAdapter
from celesto.runtime.qemu import QEMU_ROOT_NODE_NAME, QemuRuntimeAdapter
from celesto.runtime.qemu_args import build_qemu_argv
from celesto.runtime.vz import VzRuntimeAdapter
from celesto.storage import (
    SSH_PORT_END,
    SSH_PORT_START,
    MemoryStateManager,
...
from celesto.storage._base import VSOCK_CID_END, VSOCK_CID_START
...
def _usernet_addresses(backend: str, vm_id: str) -> tuple[str, str, str]:
    ...
    try:
        ...
        raise NetworkError(
            f"Celesto does not know what network settings the '{backend}' backend "
            f"gives a sandbox. Run "
            f"'celesto sandbox create --name {vm_id} --backend {BACKEND_QEMU}' instead."
        ) from None
...
def resolve_data_dir(data_dir: Path | None = None) -> Path:
    ...
    raise PermissionError(
        f"Unable to find a writable Celesto data directory. Tried: {attempted}. "
...
class CelestoManager:
    ...
    def __init__(
        ...
        """Initialize the Celesto manager.
        ...
        logger.info(
            "Celesto initialized: data_dir=%s, socket_dir=%s, backend=%s",
            self.data_dir,
            self.socket_dir,
            self.backend,
        )
    ...
    @classmethod
    def from_id(cls, vm_id: str, **kwargs: Any) -> CelestoManager:
        ...
        Returns:
            A :class:`CelestoManager` instance whose state DB contains *vm_id*.
    ...
    def __enter__(self) -> CelestoManager:
        return self
    ...
    def close(self) -> None:
        ...
        with suppress(Exception):
            self.state.close()
        self._closed = True
        logger.debug("Celesto resources released")
    ...
    def _resolve_vm_backend(self, backend_override: str | None) -> str:
        ...
        try:
            ...
            raise CelestoError(str(e)) from e
    ...
    def _backend_for_config(self, config: VMConfig) -> str:
        ...
        try:
            ...
            raise CelestoError(str(exc), {"backend": requested}) from exc
    ...
    def _runtime_adapter_for_backend(
        ...
        raise CelestoError("Unsupported backend", {"backend": backend})
    ...
    def _acquire_operation_lock(self, lock_name: str) -> tuple[Any | None, TextIO | None]:
        ...
        try:
            ...
            # Celesto currently targets POSIX hosts. If fcntl is unavailable,
    ...
    def _convert_qemu_managed_disk(
        ...
        *,
        ...
        if qemu_img is None:
            raise CelestoError("QEMU backend requires qemu-img to materialize managed disks")
    ...
    def _create_qemu_overlay_disk(
        ...
        *,
        ...
        qemu_img = self._find_qemu_img_binary()
        if qemu_img is None:
            raise CelestoError("QEMU backend requires qemu-img to create overlay disks")
    ...
    def _resize_recovery(vm_id: str) -> str:
        ...
        return f"celesto sandbox delete {vm_id}"
    ...
    def _ensure_disk_can_be_resized(config: VMConfig) -> None:
        ...
        if (config.disk_size_mib is not None or config.grow_filesystem) and (
            ...
            raise CelestoError(
                ...
                f"disk_mode='isolated', or run '{CelestoManager._resize_recovery(config.vm_id)}'."
            )
    ...
    def _run_resize_tool(
        ...
        *,
        ...
        """Run one resize helper and convert failures to CelestoError."""
    ...
    def _materialize_macos_bundle(self, config: VMConfig) -> None:
        ...
        if machine.bundle_path.name != config.vm_id:
            raise CelestoError(
                ...
                f"'celesto sandbox delete {config.vm_id}' and create it again."
            )
        ...
        if machine.bundle_path.is_symlink():
            raise CelestoError(
                ...
                f"'celesto sandbox create --os macos --name {config.vm_id}'."
            )
        ...
        from celesto.host.lume import find_lume_binary, pinned_lume_ready
        from celesto.macos.lume import LumeDriver

        binary = find_lume_binary()
        if binary is None or not pinned_lume_ready():
            raise CelestoError(
                "The macOS sandbox runtime isn't installed. Run 'celesto setup --macos', then "
                "'celesto doctor --backend vz' to confirm."
            )
        ...
        if not machine.bundle_path.is_dir():
            raise CelestoError(
                ...
                f"'celesto sandbox create --name {config.vm_id} --os macos' again."
            )
        machine.bundle_path.chmod(0o700)
    ...
    def _delete_macos_bundle(self, vm_info: VMInfo) -> None:
        ...
        if machine.bundle_path.is_symlink():
            raise CelestoError(
                ...
                f"retry 'celesto sandbox delete {vm_info.vm_id}'."
            )
    ...
    def _materialize_firmware(self, config: VMConfig) -> None:
        ...
        try:
            ...
            raise CelestoError(str(exc), {"vm_id": config.vm_id}) from exc
        ...
        # the Celesto data-dir owner only.
        target.chmod(0o600)
    ...
    def _enforce_macos_concurrency_limit(self, vm_id: str) -> None:
        ...
        if len(running) >= 2:
            ...
            raise CelestoError(
                ...
                f"'celesto sandbox stop {first}', then retry 'celesto sandbox start {vm_id}'.",
    ...
    def _check_workspace_mounts(self, vm_info: VMInfo) -> None:
        ...
        raise CelestoError(
            ...
            f"{paths}. Restore it, or run 'celesto sandbox delete {vm_info.vm_id}'.",
            {
                "vm_id": vm_info.vm_id,
    ...
    def _ensure_snapshot_supported(self, vm_info: VMInfo) -> None:
        ...
        if vm_info.config.disk_mode != "isolated":
            raise CelestoError("Snapshotting currently supports only isolated-disk VMs")
        if (
            ...
            raise CelestoError(
                ...
                f"'celesto sandbox delete {vm_info.vm_id}'."
            )
        if vm_info.config.extra_drives:
            raise CelestoError("Snapshotting currently supports only VMs without extra drives")
        if vm_info.config.workspace_mounts:
            raise CelestoError("Snapshotting is not supported for VMs with workspace mounts")
        if vm_info.network is None:
            raise CelestoError("VM has no network configuration", {"vm_id": vm_info.vm_id})
        ...
        if network.mode == "bridge":
            if vm_config is None:
                raise CelestoError(
                    ...
                    f"'celesto sandbox delete {vm_id}' to remove the restore attempt."
                )
    ...
    def _check_libkrun_prerequisites(self) -> list[str]:
        ...
        if not self._find_libkrun_library():
            errors.append(
                ...
                " run 'celesto doctor --backend libkrun' for setup guidance."
            )
    ...
    def _validate_network_policy(self, config: VMConfig, backend: str) -> None:
        ...
        validate_network_policy_options(
            ...
            recovery_command=f"celesto sandbox create --name {config.vm_id} --help",
        )
        ...
        if requested_cid is not None:
            ...
            if cid in live_cids:
                ...
                raise NetworkError(
                    ...
                    f"run 'celesto sandbox delete {vm_id}' to remove this sandbox, then create it "
    ...
    def _uses_host_tap_networking(config: VMConfig, backend: str) -> bool:
        ...
        user-owned bridge rather than Celesto's private NAT network.
        """
        if backend == BACKEND_FIRECRACKER:
            return True
        if backend == BACKEND_QEMU:
    ...
    def _validate_bridge_mode(
        ...
        nat_command = f"celesto sandbox create --name {config.vm_id} --network nat"
        if platform.system() != "Linux":
            raise CelestoError(
        ...
        if not config.guest_managed_networking:
            ...
            raise CelestoError(
                ...
                f"run 'celesto sandbox create --name {config.vm_id} --os alpine --backend "
        ...
        if resolution.kind != "vsock":
            ...
            raise CelestoError(
                ...
                f"'celesto sandbox create --name {config.vm_id} --network bridge "
        ...
        if not inspection.ok:
            raise CelestoError(inspection.reason, {"vm_id": config.vm_id, "bridge": bridge_name})
        ...
        *,
        ...
        if bridge_name != config.network_attachment.bridge:
            raise CelestoError(
                ...
                f"'celesto sandbox delete {vm_id}' to remove it.",
                {"vm_id": vm_id},
            )
    ...
    def _check_qemu_slirp_host_forward_ports(self, vm_info: VMInfo) -> None:
        ...
        if ssh_port is not None and not self._local_ssh_port_is_available(ssh_port):
            raise CelestoError(
                ...
                f"'celesto sandbox delete {vm_info.vm_id}' to remove the sandbox.",
        ...
        for forward in vm_info.config.port_forwards:
            ...
            raise CelestoError(
                ...
                f"Free that port, or run 'celesto sandbox delete {vm_info.vm_id}' "
    ...
    def create(self, config: VMConfig) -> VMInfo:
        ...
        if effective_config.workspace_mounts and backend not in {BACKEND_QEMU, BACKEND_VZ}:
            raise CelestoError(
                ...
                "re-run without '--backend' so Celesto can select a compatible runtime.",
    ...
    def start(
        ...
        Raises:
            ...
            CelestoError: If VM is not in CREATED or STOPPED state.
    ...
    def _raise_crashed_or_state_error(self, vm_info: VMInfo, *, action: str) -> None:
        ...
        if vm_info.status == VMState.ERROR:
            recovery = f"; run 'celesto sandbox delete {vm_info.vm_id}' to clear it"
        elif vm_info.status in (VMState.STOPPED, VMState.CREATED):
            recovery = f"; run 'celesto sandbox start {vm_info.vm_id}' to start it"
        raise CelestoError(
    ...
    def _create_snapshot_locked(
        ...
        *,
        ...
        backend = self._backend_for_vm(vm_info)
        disk_command = (
            f"celesto sandbox snapshot create {vm_id} --snapshot-id {snapshot_id} "
        ...
        if qemu_dirty_bitmap_backup is not None:
            ...
            if (
                ...
                if original_status != VMState.RUNNING:
                    start_command = f"celesto sandbox start {vm_id}"
                    fact = (
        ...
        if managed_disk_path is None:
            raise CelestoError("Snapshotting requires a managed isolated disk", {"vm_id": vm_id})
    ...
    def restore_snapshot(
        ...
        *,
        ...
        if snapshot.restored and not force:
            raise CelestoError("Snapshot already restored", {"snapshot_id": snapshot_id})
        ...
        if effective_snapshot.network_config.mode == "bridge":
            ...
            if configured_bridge != restored_bridge:
                raise CelestoError(
                    ...
                    f"snapshot with 'celesto sandbox snapshot delete {snapshot_id}'.",
                    {"snapshot_id": snapshot_id, "vm_id": restore_vm_id},
                )
            ...
            if effective_snapshot.backend == BACKEND_QEMU and persisted_vm_config.vsock is not None:
                try:
                    ...
                    raise NetworkError(
                        ...
                        f"run 'celesto sandbox delete {restore_vm_id}' "
    ...
    def list_snapshots(self, vm_id: str | None = None) -> list[SnapshotInfo]:
        ...
        for manifest_path in self.snapshot_dir.glob("*/manifest.json"):
            ...
            if snapshot_id in snapshots:
                continue
            with suppress(CelestoError, SnapshotNotFoundError, ValueError):
                snapshot = self._read_snapshot_manifest(snapshot_id)
    ...
    def get_ssh_commands(
        ...
        *,
        ...
        vm_info = self.state.get_vm(vm_id)
        if vm_info.network is None:
            raise CelestoError("VM has no network configuration", {"vm_id": vm_id})
        if vm_info.network.mode == "bridge":
            raise CelestoError(
                f"The host has no SSH path to bridged sandbox '{vm_id}'; use "
                f"'celesto sandbox shell {vm_id}', or connect from its bridged network.",
    ...
    def _start_qemu(
        ...
        *,
        ...
        :func:`celesto.runtime.qemu_args.build_qemu_argv`. Spawning and
        ...
        Raises:
            CelestoError: If qemu binary or required config is missing.
        """
        qemu_bin = self._find_qemu_binary()
        if qemu_bin is None:
            raise CelestoError(f"qemu-system binary is missing; {_qemu_install_hint()}")
    ...
    def _start_firecracker(
        ...
        Raises:
            CelestoError: If firecracker binary is not found.
        """
        fc_path = self.host.find_firecracker()
        if fc_path is None:
            raise CelestoError(
                "Firecracker isn't installed; run 'celesto setup', then start the sandbox again."
            )
    ...
    def _find_libkrun_library(self) -> bool:
        ...
        from celesto.runtime._libkrun_ffi import is_available
    ...
    def _build_libkrun_config(self, vm_info: VMInfo) -> dict[str, Any]:
        ...
        from celesto.runtime._libkrun_ffi import KERNEL_FORMAT_ELF, KERNEL_FORMAT_RAW
    ...
    def _start_libkrun(
        ...
        if not self._find_libkrun_library():
            raise CelestoError(
                ...
                f"run 'celesto doctor --backend libkrun', then run "
                f"'celesto sandbox start {vm_info.vm_id}'."
            )

        config_path = self._write_libkrun_config(vm_info, self._build_libkrun_config(vm_info))
        cmd = [sys.executable, "-m", "celesto.runtime._libkrun_launcher", str(config_path)]
    ...
    def _cleanup_vm_artifacts(
        ...
        *,
        ...
        ``celesto sandbox delete <id>`` able to reclaim a leaked disk.
        """
    ...
    def find_leftover_artifacts(self) -> list[LeftoverArtifact]:
        ...
        if in_use is None:
            raise CelestoError(
                ...
                "was deleted. Retry 'celesto sandbox prune' in a moment."
            )
    ...
    async def _async_unlink_socket(self, socket_path: Path) -> None:
        ...
        try:
            ...
            if result.returncode != 0:
                raise CelestoError(f"Failed to remove stale socket: {socket_path}") from None
