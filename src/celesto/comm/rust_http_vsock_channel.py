...
"""Host-side HTTP-over-vsock transport for the Rust Celesto guest agent."""
...
def _feature_required_message(feature: str, sandbox_name: str | None) -> str:
    if sandbox_name:
        ...
        return (
            ...
            f"run `celesto sandbox delete {sandbox}`, then run "
            f"`celesto sandbox create --name {sandbox}` after updating Celesto."
        )
    return (
        f"This sandbox was created from an older image and cannot use {feature}; "
        "run `celesto sandbox list`, then delete and recreate the sandbox after updating Celesto."
    )
...
def _vsock_unavailable_message(sandbox_name: str | None) -> str:
    if sandbox_name:
        ...
        return (
            ...
            f"run `celesto sandbox delete {sandbox}` and then "
            f"`celesto sandbox create --name {sandbox} --comm-channel ssh`."
        )
    return (
        "This computer cannot open fast sandbox connections; run "
        "`celesto sandbox create --comm-channel ssh` to create a sandbox with the compatible path."
    )
...
def _parse_mode_header(value: str) -> int:
    ...
    try:
        ...
        raise CelestoError(f"guest agent returned an invalid file mode: {value!r}") from exc
    if mode < 0:
        raise CelestoError(f"guest agent returned a negative file mode: {value!r}")
    return mode & 0o777
...
def _parse_size_header(value: str, *, header: str, method: str, path: str) -> int:
    ...
    if size < 0:
        raise CelestoError(f"guest agent returned negative {header} for {method} {path}")
    return size


def _feature_required_error(feature: str, *, sandbox_name: str | None = None) -> CelestoError:
    return CelestoError(_feature_required_message(feature, sandbox_name))
...
def _read_terminal_frame(
    ...
    *,
    ...
    if size > max_payload:
        raise CelestoError(f"Terminal response exceeded {max_payload} bytes.")
...
def _read_socket_line(sock: socket.socket, *, max_bytes: int = 16 * 1024) -> str:
    ...
    while not buf.endswith(b"\n"):
        if len(buf) >= max_bytes:
            raise CelestoError("Terminal handshake response was too large.")
        chunk = sock.recv(1)
        if not chunk:
...
def _run_terminal_io(
    ...
    *,
    ...
    try:
        ...
        while True:
            ...
            if sock in readable:
                ...
                if frame_type == _TERMINAL_FRAME_OUTPUT:
                    ...
                    raise CelestoError(payload.decode("utf-8", errors="replace"))
                else:
                    raise CelestoError(f"Unknown terminal response frame: {frame_type}")
        ...
        raise CelestoError("Shell connection closed before the shell exited.") from exc
...
def _safe_tar_member_mode(mode: int) -> int:
    ...
    Celesto needs root for host networking. Honouring those bits let a guest
    ...
    with tarfile.open(fileobj=io.BytesIO(data), mode="r:*") as archive:
        for member in archive.getmembers():
            ...
            if not member.isfile():
                raise CelestoError(f"Refusing unsupported tar entry: {member.name}")
            parent = target.parent
            _safe_mkdir(destination, parent)
            _reject_symlink_path(destination, target)
            source = archive.extractfile(member)
            if source is None:
                raise CelestoError(f"Tar entry has no data: {member.name}")
...
def _safe_tar_member_path(name: str) -> Path | None:
    if name.startswith(("/", "\\")):
        raise CelestoError(f"Refusing absolute tar entry: {name}")
    parts: list[str] = []
    for part in PurePosixPath(name).parts:
        if part in {"", "."}:
            continue
        if part == "..":
            raise CelestoError(f"Refusing tar entry outside destination: {name}")
        parts.append(part)
    if not parts:
        return None
    return Path(*parts)
...
def _safe_mkdir(root: Path, target: Path) -> None:
    ...
    for part in relative.parts:
        ...
        if cursor.is_symlink():
            raise CelestoError(f"Refusing tar extraction through symlink: {cursor}")
...
class RustHttpVsockChannel:
    ...
    def _open_vsock(self, port: int | None = None) -> socket.socket:
        ...
        if family is None:
            raise CelestoError(_vsock_unavailable_message(self.sandbox_name))
    ...
    def _open_uds(self, port: int | None = None) -> socket.socket:
        ...
        try:
            ...
            if not ack.startswith("OK"):
                raise CelestoError(f"vsock CONNECT handshake failed: {ack!r}")
        except Exception:
            sock.close()
            raise
        return sock
        ...
        *,
        ...
        try:
            ...
            if resp.status >= 400:
                ...
                raise CelestoError(f"guest agent HTTP {resp.status} for {method} {path}: {detail}")
            decoded = json.loads(data.decode("utf-8"))
            if not isinstance(decoded, dict):
                raise CelestoError("guest agent returned a non-object JSON response")
            return decoded
    ...
    def run(
        ...
        if not resp.get("ok"):
            raise CelestoError(f"guest agent error during run: {resp.get('error', resp)}")
    ...
    def attach_terminal(self) -> int:
        ...
        try:
            ...
            try:
                ...
                raise CelestoError("Shell needs a real terminal or pipe for input.") from exc
            ...
            if not response.get("ok"):
                raise CelestoError(str(response.get("error") or "Shell could not start."))
    ...
    def sync(self, timeout: float = 10) -> None:
        ...
        if not resp.get("ok"):
            raise CelestoError(f"guest agent error during sync: {resp.get('error', resp)}")
    ...
    def set_managed_env(self, env_vars: dict[str, str], *, merge: bool = True) -> dict[str, str]:
        ...
        if not resp.get("ok"):
            raise CelestoError(f"guest agent error during env update: {resp.get('error', resp)}")
    ...
    def list_managed_env(self) -> dict[str, str]:
        ...
        if not resp.get("ok"):
            raise CelestoError(f"guest agent error during env read: {resp.get('error', resp)}")
        vars_value = resp.get("vars")
        return dict(vars_value) if isinstance(vars_value, dict) else {}
        ...
        *,
        ...
        if not resp.get("ok"):
            ...
            if error and "timed out" not in error.lower():
                raise CelestoError(f"guest agent error during port wait: {error}")
            raise OperationTimeoutError(
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                if resp.get("status") == "ok":
                    ...
                    return
                last_error = str(resp)
            except (OSError, CelestoError, ValueError, OperationTimeoutError) as exc:
                last_error = str(exc)
