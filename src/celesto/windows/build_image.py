...
qcow2 is meant to be re-used by ``Celesto(os="windows", image=...)``,
...
def build_autounattend_iso(answer_xml: str, output_iso: Path) -> Path:
    ...
    :class:`CelestoError` with an install hint when the binary is missing.
...
class WindowsImageBuilder:
            ...
            try:
                ...
                try:
                    if vm._info.status != VMState.RUNNING:
                        ...
                        # CelestoManager.delete won't touch shared rootfs.
    ...
    def _poll_for_ready_marker(self, vm) -> None:  # noqa: ANN001 — circular Celesto type
        ...
        """
        from celesto.exceptions import CelestoError as _Err
