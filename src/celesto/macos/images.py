...
from celesto.host.lume import LUME_VERSION, find_lume_binary, pinned_lume_ready
from celesto.images.manager import resolve_image_dir
from celesto.macos.lume import LumeDriver
from celesto.macos.models import LumeVMDetails, MacOSInstallProgress, MacOSInstallRequest
...
class MacOSImageManager:
    ...
    def cached_latest_ipsw_path(self) -> Path:
        """Celesto-owned retry cache for Apple's latest restore file."""
    ...
    def find_cached_latest_ipsw(self) -> Path | None:
        """Find a complete Celesto or Lume restore download that can be reused."""
    ...
    def _adopt_latest_ipsw(self) -> Path | None:
        """Move Lume's completed temporary download into Celesto's retry cache."""
    ...
    def get(self, name: str = MACOS_DEFAULT_IMAGE) -> MacOSImageManifest:
        ...
        try:
            ...
            raise ImageError(
                ...
                f"'celesto image build --os macos --ipsw latest -t {name}'."
            ) from exc
    ...
    def _check_storage(self, *, minimum_free_bytes: int = _MINIMUM_FREE_BYTES) -> None:
        ...
        if str(details.get("FilesystemType", "")).lower() != "apfs":
            raise ImageError(
                ...
                "then run 'celesto image build --os macos --ipsw latest -t macos-latest'."
            )
        ...
        *,
        ...
        if bundle.is_dir():
            try:
                ...
                raise ImageError(
                    f"Existing macOS image files could not be verified; run 'celesto image rm "
            ...
            if (
                ...
                raise ImageError(
                    f"Existing macOS image preparation is incomplete; run 'celesto image rm "
                    f"{name}', then retry the image build."
                )
            bundle.chmod(0o700)
