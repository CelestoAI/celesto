...
"""Class-based callbacks for hooking into the Celesto command lifecycle.
...
Callbacks are attached per-VM and fire around :meth:`celesto.Celesto.run`::
    ...
    with Celesto(config, callbacks=[SafetyGuard()]) as vm:
...
class CommandBlockedError(CelestoError):
...
class Callback:
    """Base class for Celesto command-lifecycle callbacks.
...
class CallbackDispatcher:
    ...
    Internal helper used by :class:`~celesto.facade.Celesto`. ``on_pre_run`` is
