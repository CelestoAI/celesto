from http import HTTPStatus
from typing import Any, cast
from urllib.parse import quote

import httpx

from ...client import AuthenticatedClient, Client
from ...types import Response, UNSET
from ... import errors

from ...models.agent_builder_response import AgentBuilderResponse
from ...models.agent_builder_update_request import AgentBuilderUpdateRequest
from ...models.http_validation_error import HTTPValidationError
from typing import cast


def _get_kwargs(
    config_id: str,
    *,
    body: AgentBuilderUpdateRequest,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {
        "method": "put",
        "url": "/v1/builder/{config_id}".format(
            config_id=quote(str(config_id), safe=""),
        ),
    }

    _kwargs["json"] = body.to_dict()

    headers["Content-Type"] = "application/json"

    _kwargs["headers"] = headers
    return _kwargs


def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> AgentBuilderResponse | HTTPValidationError | None:
    if response.status_code == 200:
        response_200 = AgentBuilderResponse.from_dict(response.json())

        return response_200

    if response.status_code == 422:
        response_422 = HTTPValidationError.from_dict(response.json())

        return response_422

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[AgentBuilderResponse | HTTPValidationError]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    config_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: AgentBuilderUpdateRequest,
) -> Response[AgentBuilderResponse | HTTPValidationError]:
    """Update an agent config

     Update an agent config.

    Args:
        config_id (str):
        body (AgentBuilderUpdateRequest): Request schema for updating an existing agent config.

            Only name and config_data can be updated. Description is set at creation
            time and cannot be modified afterwards (by design - descriptions serve as
            immutable documentation of the config's original purpose).

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[AgentBuilderResponse | HTTPValidationError]
    """

    kwargs = _get_kwargs(
        config_id=config_id,
        body=body,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    config_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: AgentBuilderUpdateRequest,
) -> AgentBuilderResponse | HTTPValidationError | None:
    """Update an agent config

     Update an agent config.

    Args:
        config_id (str):
        body (AgentBuilderUpdateRequest): Request schema for updating an existing agent config.

            Only name and config_data can be updated. Description is set at creation
            time and cannot be modified afterwards (by design - descriptions serve as
            immutable documentation of the config's original purpose).

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        AgentBuilderResponse | HTTPValidationError
    """

    return sync_detailed(
        config_id=config_id,
        client=client,
        body=body,
    ).parsed


async def asyncio_detailed(
    config_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: AgentBuilderUpdateRequest,
) -> Response[AgentBuilderResponse | HTTPValidationError]:
    """Update an agent config

     Update an agent config.

    Args:
        config_id (str):
        body (AgentBuilderUpdateRequest): Request schema for updating an existing agent config.

            Only name and config_data can be updated. Description is set at creation
            time and cannot be modified afterwards (by design - descriptions serve as
            immutable documentation of the config's original purpose).

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[AgentBuilderResponse | HTTPValidationError]
    """

    kwargs = _get_kwargs(
        config_id=config_id,
        body=body,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    config_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: AgentBuilderUpdateRequest,
) -> AgentBuilderResponse | HTTPValidationError | None:
    """Update an agent config

     Update an agent config.

    Args:
        config_id (str):
        body (AgentBuilderUpdateRequest): Request schema for updating an existing agent config.

            Only name and config_data can be updated. Description is set at creation
            time and cannot be modified afterwards (by design - descriptions serve as
            immutable documentation of the config's original purpose).

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        AgentBuilderResponse | HTTPValidationError
    """

    return (
        await asyncio_detailed(
            config_id=config_id,
            client=client,
            body=body,
        )
    ).parsed
