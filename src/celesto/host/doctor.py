...
"""Host diagnostics for Celesto (``celesto doctor``)."""
...
from celesto.cli.output import console_stdout, emit_json, render_error, status_style
from celesto.exceptions import CelestoError
from celesto.host.manager import HostManager
from celesto.host.network import check_network_prerequisites
from celesto.host.paths import configured_firecracker_dir
    ...
    try:
        ...
        return DoctorCheck(
            ...
            detail=(
                ...
                f"{_QEMU_PROBE_TIMEOUT_SECONDS} seconds. Run 'celesto doctor' again."
            ),
        )
...
class WorkerNodeSecurityError(CelestoError):
...
def _doctorize_worker_security_check(check: DoctorCheck) -> DoctorCheck:
    ...
    Local users should be able to validate whether Celesto can run on their
...
def check_worker_node_security() -> list[DoctorCheck]:
    ...
    Example usage in a reconciler entrypoint::
        ...
        from celesto.host.doctor import check_worker_node_security, WorkerNodeSecurityError
...
def generate_doctor_report(backend: str | None = None) -> DoctorReport:
    ...
    if resolved == BACKEND_FIRECRACKER:
        ...
        setup_command = (
            ...
            else f"celesto setup --firecracker-dir {shlex.quote(str(configured_dir))}"
        )
        checks.append(
            DoctorCheck(
                name="firecracker",
        ...
        checks.append(
            DoctorCheck(
                ...
                fix=None
                if is_apple_silicon
                else "Run 'celesto sandbox create --os alpine' on this machine instead.",
            )
        )
            ...
            try:
                ...
                checks.append(
                    DoctorCheck(
                        ...
                        fix=None if runtime_ok else "Run 'celesto setup --macos'.",
                    )
                )
        ...
        from celesto.runtime._libkrun_ffi import is_available as _libkrun_available
