...
def find_lume_binary() -> Path | None:
    """Find the checksum-verified Lume release managed by Celesto."""
...
def _verify_lume_app(app_path: Path) -> None:
    ...
    for command, label in checks:
        try:
            ...
            raise HostError(
                f"Could not verify the macOS runtime {label}; run 'celesto setup --macos' again."
            ) from exc
        if result.returncode != 0:
            raise HostError(
                f"The macOS runtime failed {label} verification; run 'celesto setup --macos' again."
            )
...
def install_pinned_lume(*, destination: Path = LUME_MANAGED_PATH) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="smolvm-lume-", dir=str(install_root.parent)) as tmp:
        ...
        if actual != LUME_ARCHIVE_SHA256:
            raise HostError(
                ...
                "'celesto setup --macos' to download it again."
            )
        staged.mkdir()
    ...
    if installed_version != LUME_VERSION:
        ...
        raise HostError(
            ...
            f"{LUME_VERSION!r}; run 'celesto setup --macos' to reinstall it."
        )
    subprocess.run(
