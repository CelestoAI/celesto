...
"""Host setup runner for the ``celesto setup`` CLI command."""
...
class SetupOptions:
    """Resolved CLI options for ``celesto setup``."""
...
def packaged_asset_root() -> Path:
    ...
    Checks the installed ``celesto._setup_assets`` package first, then falls
    back to the repository ``scripts/`` directory so ``uv run celesto setup``
    ...
    # src/celesto/host/setup.py → src/celesto/host → src/celesto → src → repo root.
...
def _package_asset_root() -> Path | None:
    ...
    try:
        return Path(fspath(files("celesto._setup_assets")))
...
def detect_setup_platform(system_name: str | None = None) -> SetupPlatform:
    ...
    if detected == "Darwin":
        return "macos"
    raise RuntimeError(
        f"`celesto setup` is supported only on Linux and macOS. Detected OS: {detected}."
    )
...
def build_setup_command(
    ...
    *,
    ...
    """Build the child ``bash ...`` invocation for ``celesto setup``."""
