...
from celesto.exceptions import BridgeTapOwnershipError, CelestoError, NetworkError
from celesto.host._accel import HAS_NETLINK, network_native
from celesto.utils import async_run_command, run_command
...
def _mark_native_unprivileged() -> None:
    ...
    if not _native_unprivileged:
        ...
        logger.warning(
            ...
            "directly (root or CAP_NET_ADMIN). Celesto is using the slower sudo "
            "fallback; run `celesto setup` if sudo fallback is missing, or start "
            "the same Celesto command with sudo to get the Rust networking speedup."
        )
...
class BridgeInspection:
    ...
    Attributes:
        ...
        ok: Whether the bridge is valid for Celesto attachment.
...
class NetworkManager:
        ...
        for attempt in range(max_busy_retries + 1):
            try:
                ...
                if is_busy and attempt < max_busy_retries:
                    ...
                    continue
                raise
        raise CelestoError(f"Failed to create TAP device '{tap_name}'")
    ...
    def _ensure_nftables_base(self) -> None:
        """Create Celesto nftables tables/chains if missing.
    ...
    async def _async_ensure_nftables_base(self) -> None:
        """Create Celesto nftables tables/chains if missing (async).
    ...
    def _check_local_port_ownership(
        ...
        for line in output.splitlines():
            ...
            raise NetworkError(
                ...
                f"'celesto sandbox port expose {vm_id} {guest_port}'."
            )
    ...
    def setup_local_port_forward(
        ...
        try:
            ...
            raise NetworkError(
                ...
                f"'celesto sandbox port expose {vm_id} {host_port}:{guest_port}'."
            ) from exc
    ...
    def apply_egress_allowlist(
        ...
        Installs per-TAP rules in the Celesto filter forward chain keyed by tap
    ...
    def inspect_bridge(self, bridge_name: str) -> BridgeInspection:
        """Read-only validation of a Linux bridge for Celesto attachment."""
        import platform
        ...
        if platform.system() != "Linux":
            return BridgeInspection(
                ...
                reason=(
                    ...
                    "'celesto sandbox create --network nat' instead."
                ),
            )
        ...
        if link_info is None:
            return BridgeInspection(
                ...
                f"'celesto bridge check {bridge_name}'.",
            )
        ...
        if kind != "bridge":
            ...
            return BridgeInspection(
                ...
                f"bridge, then run 'celesto bridge check {bridge_name}'.",
            )
        ...
        if not external_members:
            return BridgeInspection(
                ...
                f"its VLAN or physical interface, then run 'celesto bridge check {bridge_name}'.",
            )
    ...
    def _require_owned_bridge_tap(self, tap_name: str, vm_id: str) -> dict[str, object]:
        ...
        if kind != "tun" or tap_type != "tap":
            raise BridgeTapOwnershipError(
                f"Network interface '{tap_name}' already exists and is not a Celesto TAP; "
        ...
        try:
            ...
            if master != bridge_name:
                raise NetworkError(
                    ...
                    f"run 'celesto sandbox delete {vm_id}', then retry."
                )
        except Exception:
            if created:
    ...
    def cleanup_bridged_tap(self, tap_name: str, vm_id: str) -> None:
        ...
        raise NetworkError(
            ...
            f"'celesto sandbox delete {vm_id}' again."
        )
    ...
    if os.geteuid() != 0:
        ...
        for cmd, label in checks:
            try:
                ...
                errors.append(f"{label} missing (run `celesto setup`)")
