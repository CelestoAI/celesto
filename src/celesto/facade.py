...
"""User-friendly VM facade matching the Celesto Initial Design API.

Provides a simple ``Celesto`` class that wraps the lower-level
:class:`~celesto.vm.CelestoManager` manager, giving callers an instance-style
interface::
...
from celesto.env import inject_env_vars, read_env_vars, remove_env_vars
...
from celesto.images.manager import ImageManager
...
from celesto.ssh import ShellKind, SSHClient
...
def _build_local_image_config(
    *,
    ...
    ``Celesto(image=...)`` calls don't collide on the write lock and the
    ...
    if os_input is None:
        raise ValueError(
            'Local images need os= (e.g. Celesto(os="windows", image="..."))'
            " so Celesto knows which operating system is inside the file."
        )
...
def _build_auto_config(
    *,
    ...
    if resolved_os is GuestOS.UBUNTU:
        from celesto.images.published import Vmm, ensure_published_image
        ...
        if disk_size_mib is None:
            ...
            raise ValueError(
                ...
                f"'{resolved_vm_name}'; "
                f"recreate it with: celesto sandbox create --name {quoted_vm_name} --os ubuntu "
...
class Celesto:
    ...
    Create a VM with a config or call ``Celesto()`` for an
    ...
    Args:
        config: VM configuration. Mutually exclusive with *vm_id* and *image*.
            If omitted (and *vm_id* is omitted), Celesto auto-creates
        ...
        memory: Guest memory in MiB for auto-config mode (``Celesto()`` only).
        disk_size: Root filesystem size in MiB for auto-config mode (``Celesto()`` only).
        ...
        ssh_password: Optional SSH password (paramiko password auth).
            ...
            Celesto clears the key path internally so the password is
            actually tried.)
        internet_settings: Network access controls. Accepts an
            :class:`~celesto.types.InternetSettings` instance or a dict
        ...
        writable_mounts: When ``True``, every entry in *mounts* is
            ...
            :attr:`~celesto.types.WorkspaceMount.writable` directly on
        ...
        *,
        ...
        if vm_id is not None and state_manager is None:
            raise ValueError(
                ...
                "Celesto object, or use the Celesto CLI for persistent sandboxes."
            )
        ...
        # (on the kwarg or the config) still get the CelestoManager error if
        ...
        if config is not None:
            self._sdk = CelestoManager(**sdk_kwargs)
            ...
            self._sdk = CelestoManager.from_id(vm_id, **sdk_kwargs)
    ...
    def desktop(
        ...
        *,
        ...
        from celesto.browser import _DesktopSandbox
    ...
    def computer(
        ...
        *,
        ...
        :class:`Celesto` constructor when no graphical interface is needed.
        """
    ...
    def from_id(
        ...
        *,
        ...
        Returns:
            A :class:`Celesto` instance bound to the existing VM.
    ...
    def start(
        ...
        *,
        ...
        Raises:
            CelestoError: If ``env_vars`` is set but the image does not
    ...
    def snapshot(
        ...
        *,
        ...
        if resolved_capture_policy == SnapshotCapturePolicy.LIVE_ONLY:
            live_command = (
                f"celesto sandbox snapshot create {self._vm_id} --snapshot-type disk "
    ...
    def _sync_guest_for_disk_snapshot(self) -> None:
        ...
        try:
            ...
            raise CelestoError(
                ...
                f"files inside it failed; retry with 'celesto sandbox snapshot create "
    ...
    def _prepare_run(
        ...
        if self._info.status != VMState.RUNNING:
            raise CelestoError(
                ...
                f"'celesto sandbox start {self._vm_id}' "
        ...
        if resolution.kind == "ssh" and self._info.network is None:
            raise CelestoError(
                ...
                f"no network; remove it with 'celesto sandbox delete {self._vm_id}' "
                "and create it again.",
                {"vm_id": self._vm_id},
            )
    ...
    def upload_file(
        ...
        *,
        ...
        Raises:
            ...
            CelestoError: If the VM is not running or file transfer fails.
        """
        ...
        *,
        ...
        Raises:
            ValueError: If *guest_path* is empty or not an absolute
                POSIX path.
            CelestoError: If the VM is not running, the local parent
    ...
    def wait_for_ready(
        ...
        *,
        ...
        Raises:
            ...
            CelestoError: If the VM is not running.
        """
        self._refresh_info()
        ...
        if self._resolve_channel().kind == "ssh" and self._info.network is None:
            raise CelestoError(
                ...
                f"remove it with 'celesto sandbox delete {self._vm_id}' and create it again.",
                {"vm_id": self._vm_id},
            )
    ...
    def _ensure_shell_supported(self) -> None:
        ...
        if self._resolve_channel().kind != "vsock":
            raise CelestoError(
                f"Sandbox '{self._vm_id}' cannot use 'celesto sandbox shell'; "
                f"run 'celesto sandbox ssh {self._vm_id}' to open an SSH shell.",
                {"vm_id": self._vm_id},
            )

    def ensure_shell_supported(self) -> Celesto:
        """Verify that this sandbox can use ``celesto sandbox shell``."""
    ...
    def _ensure_shell_control(self, *, timeout: float) -> CommChannel:
        ...
        if not callable(attach_terminal):
            raise CelestoError(
                ...
                f"run 'celesto sandbox ssh {self._vm_id}' to use SSH.",
                {"vm_id": self._vm_id},
            )
        return channel

    def wait_for_shell(self, *, timeout: float = _DEFAULT_RUN_READY_TIMEOUT) -> Celesto:
    ...
    def _ssh_direct_command(self) -> list[str]:
        ...
        # Resolve key: explicit > default Celesto key.
        ...
        if key_path:
            ...
            # for the auto-resolved Celesto key, allow agent/default auth
            # as a fallback.
            if explicit_key:
        ...
        *,
        ...
        if self._info.network.mode == "bridge":
            raise CelestoError(
                ...
                f"network; run 'celesto sandbox delete {self._vm_id}', then "
                f"'celesto sandbox create --name {self._vm_id} --network nat' to use port "
                "exposure.",
    ...
    def unexpose_local(self, host_port: int, guest_port: int) -> Celesto:
    ...
    def open_desktop(self) -> DesktopEndpoint:
        ...
        if self._info.status is not VMState.RUNNING:
            raise CelestoError(
                ...
                f"'celesto sandbox start {self._vm_id}', then "
                f"'celesto sandbox desktop {self._vm_id}'."
            )
        endpoint = self._info.display
        if endpoint is None:
            raise CelestoError(
                f"Sandbox '{self._vm_id}' has no desktop; run "
                f"'celesto sandbox shell {self._vm_id}' to use its terminal."
            )
        ...
        if machine is not None:
            ...
            try:
                ...
                raise CelestoError(
                    ...
                    f"'celesto sandbox desktop {self._vm_id}' to retry."
                ) from exc
    ...
    def get_ip(self) -> str:
        ...
        Raises:
            CelestoError: If the VM has no network configuration, or if the
        ...
        if self._info.network.mode == "bridge":
            raise CelestoError(
                ...
                f"use 'celesto sandbox shell {self._vm_id}', or connect to its "
                f"address from the bridged network.",
                {"vm_id": self._vm_id, "network_mode": "bridge"},
            )
        return self._info.network.guest_ip

    def refresh(self) -> Celesto:
    ...
    async def async_start(self, boot_timeout: float = 30.0) -> Celesto:
    ...
    async def async_wait_for_ready(self, timeout: float = 60.0) -> Celesto:
    ...
    async def async_wait_for_ssh(self, timeout: float = 60.0) -> Celesto:
    ...
    async def async_create_many(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            **kwargs: Passed to Celesto constructor.

        Returns:
            List of started Celesto instances.
        """
        import asyncio

        sem = asyncio.Semaphore(concurrency or len(configs))

        async def create_one(config: VMConfig) -> Celesto:
            async with sem:
    ...
    async def __aenter__(self) -> Celesto:
...
modprobe 9pnet_virtio""".strip()
    ...
    def _managed_env_required_error(self) -> CelestoError:
        """Return the user-facing error for vsock env without managed support."""
        recovery_command = f"celesto sandbox create --name {self._vm_id} --comm-channel ssh"
        return CelestoError(
            f"Managed environment variables are not available for sandbox "
            f"'{self._vm_id}' over vsock; run 'celesto sandbox delete {self._vm_id}' "
    ...
    def _ssh_endpoints(self) -> list[tuple[str, int]]:
        ...
        if self._info.network.mode == "bridge":
            raise CelestoError(
                ...
                f"'celesto sandbox shell {self._vm_id}', or connect from its bridged network.",
                {"vm_id": self._vm_id, "network_mode": "bridge"},
            )
    ...
    def _try_vsock_ready(self, timeout: float) -> bool:
        ...
        except (OperationTimeoutError, CelestoError, OSError):
            channel.close()
            return False
    ...
    def _wait_for_ready(self, timeout: float) -> None:
        ...
        if resolution.kind == "vsock":
            ...
            raise OperationTimeoutError(
                ...
                f"'celesto sandbox delete {self._vm_id}' and then "
                f"'celesto sandbox create --name {self._vm_id} --comm-channel ssh' "
    ...
    def _command_exec_remediation(self) -> str:
        ...
        return (
            "Use `Celesto()` auto-config mode for an SSH-ready image, or build one with "
            "`ImageBuilder` when you need custom guest composition."
        )

    def __repr__(self) -> str:
        return f"Celesto(vm_id={self._vm_id!r}, status={self._info.status.value!r})"
