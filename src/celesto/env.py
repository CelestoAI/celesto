...
"""Environment variable injection for Celesto guests.
...
def build_env_script(env_vars: dict[str, str]) -> str:
    ...
    """
    if not env_vars:
        return "# Celesto environment variables (empty)\n"

    lines = ["#!/bin/sh", "# Celesto managed environment variables", ""]
...
def inject_env_vars(
    ...
    *,
    ...
    Raises:
        CelestoError: If the SSH command fails.
...
def _atomic_write(ssh: CommChannel, content: str) -> "CommandResult":  # noqa: F821
    ...
    """
    from celesto.types import CommandResult  # noqa: F401 - avoid circular import
