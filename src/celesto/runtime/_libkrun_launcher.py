...
Invoked as ``python -m celesto.runtime._libkrun_launcher <config.json>``.
...
from celesto.runtime._libkrun_ffi import KERNEL_FORMAT_RAW, KrunContext, _libkrun
...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        print("usage: python -m celesto.runtime._libkrun_launcher <config.json>", file=sys.stderr)
        return 2
