...
Splits the QEMU argv builder out of ``CelestoManager._start_qemu`` so it can
...
from celesto.runtime.qemu import QEMU_ROOT_NODE_NAME
from celesto.types import GuestOS, QemuMachine, VMInfo
...
def build_qemu_argv(
    ...
    *,
    ...
    Raises:
        CelestoError: When the VM has no network config; when slirp mode has
    ...
    if vm_info.network is None:
        raise CelestoError("QEMU backend requires a VM network config")
    if (
        not tap_mode
        ...
        raise CelestoError(
            ...
            f"'celesto sandbox create --name {vm_info.vm_id} --comm-channel ssh'."
        )
