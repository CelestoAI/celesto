...
from celesto.qmp import QMPClient, QMPJobFailedError, QMPJobTimeoutError
...
from celesto.runtime.guest_platforms import GuestPlatformSpec, get_guest_platform
...
from celesto.utils import which
...
def _live_backup_operation_id(job_id: str, target_node: str) -> str | None:
    """Return a validated operation suffix for one Celesto QMP resource pair."""
...
class _SwtpmSidecar:
    ...
    def start(self, *, timeout: float = 5.0) -> int:
        ...
        Raises:
            CelestoError: When ``swtpm`` isn't on PATH, when the swtpm
...
class QemuRuntimeAdapter(RuntimeAdapter):
    ...
    def _resolve_platform_spec(vm_info: VMInfo) -> GuestPlatformSpec:
        ...
        host-incompatibility raised by the spec factory) as a ``CelestoError``
        with the VM ID in context, so the runtime adapter's error path
        is consistent with the rest of the Celesto SDK.
        """
        try:
    ...
    def _live_backup_fallback_command(request: SnapshotCreateRequest) -> str:
        ...
        return (
            f"celesto sandbox snapshot create {request.vm_info.vm_id} "
    ...
    def _bitmap_recovery_clear_command(snapshot_id: str) -> str:
        ...
        return f"celesto sandbox snapshot delete {snapshot_id}"
    ...
    def _bitmap_recovery_error(
        ...
        *,
        ...
        return CelestoError(f"{fact} {recovery}", error_details)
    ...
    def _create_live_disk_snapshot(
        ...
        try:
            ...
            if cleanup_safe:
                with suppress(OSError):
                    manifest_path.unlink()
                if isinstance(original_error, CelestoError) and original_error.details.get(
                    "recovery_command"
                ):
                    raise
                raise self._live_backup_error(
        ...
        if snapshot.snapshot_type == SnapshotType.DIFF:
            ...
            if backing is not None and not backing.exists():
                raise CelestoError(
                    ...
                    f"'celesto sandbox snapshot restore {snapshot.snapshot_id}' again, or take "
    ...
    def _client(self, control_socket_path: Path | None, timeout: float = 5.0) -> QMPClient:
        ...
        if control_socket_path is None:
            raise CelestoError("VM has no QMP socket path")
