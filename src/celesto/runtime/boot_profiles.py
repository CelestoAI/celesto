...
"""Internal kernel boot-profile definitions for Celesto images.

After 0.0.14a0 Celesto ships a single universal microvm kernel
...
the same across all Celesto-built profiles.
"""
...
from celesto.runtime.backends import BACKEND_FIRECRACKER, BACKEND_LIBKRUN, BACKEND_QEMU

if TYPE_CHECKING:
    from celesto.images.published import Arch as PublishedArch
...
def normalize_arch(arch: str) -> str:
    """Normalize host architecture values to Celesto kernel keys."""
...
def to_published_arch(arch: str) -> PublishedArch:
    """Map kernel-style arch (`x86_64`/`aarch64`) to Celesto-style (`amd64`/`arm64`).
...
def resolve_kernel_path(
    ...
    *,
    ...
    """Return the local path to the Celesto-built base kernel.
    ...
    the container.
    """
    from celesto.images.published import _kernel_format_for_vmm, ensure_base_kernel
