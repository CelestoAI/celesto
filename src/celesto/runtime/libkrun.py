...
from celesto.runtime.backends import BACKEND_LIBKRUN
...
class LibkrunRuntimeAdapter(RuntimeAdapter):
    ...
    def start(self, vm_info: VMInfo, *, log_path: Path, boot_timeout: float) -> RuntimeLaunch:
        if self._context.start_libkrun is None:
            raise CelestoError("libkrun launch function is not configured in runtime context")
    ...
    def pause(self, vm_info: VMInfo) -> None:
        raise CelestoError("libkrun backend does not support pause yet")

    def resume(self, vm_info: VMInfo) -> None:
        raise CelestoError("libkrun backend does not support resume yet")

    def create_snapshot(self, request: SnapshotCreateRequest) -> SnapshotCreateResult:
        raise CelestoError("libkrun backend does not support snapshots yet")

    def restore_snapshot(self, request: SnapshotRestoreRequest) -> RuntimeLaunch:
        raise CelestoError("libkrun backend does not support snapshot restore yet")
    ...
    def _wait_for_runtime(self, process: Any, boot_timeout: float, vm_id: str) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if rc is not None:
                raise CelestoError(
                    ...
                    f"Run 'celesto sandbox delete {vm_id}' to remove it.",
        ...
        if rc is not None:
            ...
            raise CelestoError(
                f"VM '{vm_id}' failed to start. Run 'celesto sandbox delete {vm_id}' to remove it.",
                {"return_code": rc, "pid": process.pid},
            )
