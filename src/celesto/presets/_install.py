...
"""Apply a preset to a running Celesto guest over its control channel."""
...
from celesto.env import inject_env_vars
from celesto.exceptions import CelestoError
from celesto.presets._git import GIT_HOST_CONFIGS, register_workspace_safe_directories
...
def _run_install_phase(
    ...
    *phase* tags the CelestoError context (``"setup"`` or ``"install"``) so
    ...
    recovery_parts = ["celesto", public_name, "start"]
    if sandbox_name:
...
def transfer_host_configs(
    ...
    *,
    ...
    which raises :class:`CelestoError`. Returns the guest paths written.
    """
    notify = on_progress or (lambda _msg: None)
