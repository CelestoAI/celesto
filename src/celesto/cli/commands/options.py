"""Small Click helpers shared by Celesto commands."""
    ...
    def handle_parse_result(
        ...
        if self.name in opts and platform.system() != "Linux":
            ...
            raise click.UsageError(
                f"{option} is only supported on Linux. Run 'celesto setup' without this flag."
            )
...
def _complete_from_state(select: Callable[[Any], Any], incomplete: str) -> list[Any]:
    ...
    Opens the state store directly (not CelestoManager, which would create
