...
from celesto.cli._kvm_session import maybe_reexec_for_kvm_group
...
from celesto.types import BrowserSessionState, DesktopEndpoint, GuestOS, VMState
...
class ListPayload(TypedDict):
    """JSON payload for ``celesto sandbox list``."""
...
class CreateVmPayload(TypedDict):
    """Machine-readable VM details for ``celesto sandbox create``."""
...
class CreateNextPayload(TypedDict):
    """Suggested follow-up actions for ``celesto sandbox create``."""
...
class InfoVmPayload(TypedDict):
    """Machine-readable VM details for ``celesto sandbox info``.

    Memory and disk fields are in MiB (Celesto's house unit, matching
...
class InfoPayload(TypedDict):
    """JSON payload for ``celesto sandbox info``."""

    vm: InfoVmPayload


class CreatePayload(TypedDict):
    """JSON payload for ``celesto sandbox create``."""
...
class OpenClawOpenUiPayload(TypedDict):
    """Machine-readable result for ``celesto openclaw open-ui``."""
...
class SnapshotListPayload(TypedDict):
    """JSON payload for ``celesto sandbox snapshot list``."""
...
class SnapshotRestoreVmPayload(TypedDict):
    """Machine-readable VM details for ``celesto sandbox snapshot restore``."""
...
class SnapshotRestorePayload(TypedDict):
    """JSON payload for ``celesto sandbox snapshot restore``."""
...
class FileUploadPayload(TypedDict):
    """JSON payload for ``celesto sandbox file upload``."""
...
class FileDownloadPayload(TypedDict):
    """JSON payload for ``celesto sandbox file download``."""
...
class BrowserListPayload(TypedDict):
    """JSON payload for ``celesto browser list``."""
...
class BrowserSandboxPayload(TypedDict):
    """Machine-readable sandbox details for ``celesto browser start``."""
    ...
    for mount in vm.config.workspace_mounts:
        if not mount.host_path.exists():
            warnings.append(
                ...
                f"'celesto sandbox delete {vm.vm_id}' to remove the sandbox."
            )
    return warnings
...
def _render_list(rows: list[VmRow], *, show_preset_column: bool = True) -> None:
    ...
    from celesto.presets import public_preset_name

    table = Table(title="Celesto Instances")
...
def _run_setup(
    *,
    ...
    from celesto.host.setup import SetupOptions, packaged_asset_root, run_setup
    ...
    if macos_runtime:
        ...
        try:
            ...
            if check_only:
                from celesto.runtime.backends import BACKEND_VZ, ensure_backend_available
                ...
                if binary is None or lume_version(binary) != LUME_VERSION:
                    raise RuntimeError(
                        ...
                        "'celesto setup --macos' to install it."
                    )
    ...
    if options.for_bake:
        console_stdout().print(
            ...
            "Run 'celesto doctor' on the runtime host before booting VMs.[/yellow]"
        )
    ...
    with _cli_service().manager() as sdk:
        try:
            ...
            if not vms:
                if preset_filter is not None:
                    ...
                    if include_all:
                        message += (
                            ...
                            "\n'celesto sandbox list --all'; to create one, run "
                            f"'celesto {public_name} start'."
                        )
                    else:
                        filtered_list_command = (
                            f"celesto {public_name} list"
                            if command_name == f"{public_name}.list"
                            else f"celesto sandbox list --preset {public_name}"
                        )
                ...
                render_empty("Celesto Instances", message)
                return 0
...
def _query_live_vm_info(vm: VMInfo) -> dict[str, object]:
    ...
    blocking on the Celesto facade's 30-second SSH-ready wait.
...
def _disk_size_mib(rootfs_path: Path | None) -> int | None:
    ...
    if rootfs_path.suffix.lower() == ".qcow2":
        from celesto.facade import _qcow2_virtual_size_mib
...
def _build_and_boot_with_progress(
    *,
    ...
    check completes.
    Returns a started :class:`~celesto.facade.Celesto` instance.
    """
...
def _wait_after_create(
    ...
    *,
    ...
    """Wait for the channel promised by ``celesto sandbox create``."""
...
def _apply_network_attachment(
    ...
    *,
    ...
    if network_mode == "bridge":
        ...
        from celesto.types import NetworkAttachmentConfig
...
def _run_create(args: SimpleNamespace) -> int:
    """Handle ``celesto sandbox create``."""
    from celesto.facade import (
        Celesto,
        _build_auto_config,
    ...
    try:
        ...
        if resolved_guest_os is GuestOS.MACOS and not use_s3_image:
            ...
            from celesto.macos.images import MACOS_DEFAULT_IMAGE, MacOSImageManager
            ...
            try:
                ...
                if existing_bundle:
                    ...
                    preparation_summary = (
                        "Celesto will download macOS from Apple and create a reusable local image; "
        ...
        # and apt cache don't fill the rootfs on a basic `celesto sandbox create`.
        ...
        data: CreatePayload = {
            ...
            "next": {
                "shell_command": (None if is_macos else f"celesto sandbox shell {vm.vm_id}"),
                "ssh_command": (
                    None if is_bridge or is_macos else f"celesto sandbox ssh {vm.vm_id}"
                ),
                "desktop_command": (f"celesto sandbox desktop {vm.vm_id}" if is_macos else None),
                "info_command": f"celesto sandbox info {vm.vm_id}",
            },
            "warnings": (
                [
                    f"The guest manages its own network address; if it did not receive one, "
                    f"run 'celesto sandbox shell {vm.vm_id}' to configure it."
                ]
                if is_bridge
                else []
            ),
...
# vmm → Celesto runtime backend. libkrun ships a Firecracker-API-compatible
...
def _run_start_with_published_image(args: SimpleNamespace, preset: object) -> int:
    ...
    """
    from celesto.exceptions import ImageError
    from celesto.facade import Celesto, _resolve_vm_name
    from celesto.images.published import ensure_published_image
    ...
    try:
        ...
        try:
            ...
            data: StartPayload = {
                ...
                "next": {
                    "shell_command": f"celesto sandbox shell {vm.vm_id}",
                    "ssh_command": f"celesto sandbox ssh {vm.vm_id}",
...
def _run_start(args: SimpleNamespace) -> int:
    ...
    if requested_os.value not in preset.supported_oses:
        ...
        return _emit_cli_error(
            ...
            2,
            ValueError(
                f"{preset.name} runs on {supported_os} in Celesto; run "
                f"'celesto {command_name.rsplit('.', 1)[0]} start --os {supported_os}'."
            ),
            json_output=args.json,
        )
...
def _maybe_attach_and_launch(
    ...
    *attach* tri-state: ``True`` skip prompt and attach; ``False`` skip
    ...
    _vm: Celesto = vm  # type: ignore[assignment]
    _preset: Preset = preset  # type: ignore[assignment]
...
def _exec_launch_command(vm: object, launch_command: str) -> int:
    ...
    from celesto.env import ENV_FILE
    from celesto.facade import Celesto
...
def _apply_preset_with_progress(
    *,
    ...
    from celesto.facade import Celesto
    from celesto.presets import apply_preset
    from celesto.presets._types import Preset
...
def _render_snapshot_list(rows: list[SnapshotRow]) -> None:
    ...
    table = Table(title="Celesto Snapshots")
...
def _run_desktop(args: SimpleNamespace) -> int:
    """Handle ``celesto sandbox desktop``."""
    ...
    try:
        ...
        if vm.status is not VMState.RUNNING:
            raise CelestoError(
                ...
                f"'celesto sandbox start {vm.vm_id}', then "
                f"'celesto sandbox desktop {vm.vm_id}'."
            )
        if endpoint is None:
            raise CelestoError(
                f"Sandbox '{vm.vm_id}' has no desktop; run "
                f"'celesto sandbox shell {vm.vm_id}' to use its terminal."
            )
...
def _run_snapshot(args: SimpleNamespace) -> int:
    """Handle ``celesto sandbox snapshot`` commands."""
    from celesto.facade import Celesto
    ...
    if args.snapshot_action is None:
        render_error(
            "Usage: celesto sandbox snapshot {create,restore,delete,list} ... "
            "Run 'celesto sandbox snapshot --help' for usage."
        )
        return 2
    ...
    if args.snapshot_action == "create":
        vm: Celesto | None = None
        try:
            from celesto.types import SnapshotCapturePolicy
    ...
    try:
        ...
        if not rows:
            if args.vm_id:
                render_empty("Celesto Snapshots", f"No snapshots found for VM '{args.vm_id}'.")
            else:
                render_empty("Celesto Snapshots", "No snapshots found.")
            return 0
...
def _render_env_list(vm_id: str, data: dict[str, object]) -> None:
    ...
    if not variables:
        render_empty(
            ...
            f"No Celesto-managed environment variables on '{vm_id}'.",
        )
        return
...
def _run_env(args: SimpleNamespace) -> int:
    """Handle ``celesto sandbox env set|unset|list``."""
    from celesto.facade import Celesto

    if args.env_action is None:
        render_error(
            "Usage: celesto sandbox env {set,unset,list} <vm_id> ... "
            "Run 'celesto sandbox env --help' for usage."
        )
        return 2
...
def _run_file(args: SimpleNamespace) -> int:
    """Handle ``celesto sandbox file`` commands."""
    from celesto.facade import Celesto

    if args.file_action is None:
        render_error(
            "Usage: celesto sandbox file {upload,download} ... "
            "Run 'celesto sandbox file --help' for usage."
        )
        return 2
...
def _hint_if_vm_crashed(vm: object) -> None:
    ...
    """
    from celesto.facade import Celesto
    from celesto.vm import _crashed_message
    ...
    *,
    ...
    if vm.status in {VMState.CREATED, VMState.STOPPED}:
        ...
        raise RuntimeError(
            ...
            f"'celesto sandbox logs {vm.vm_id}', or run "
            f"'celesto sandbox delete {vm.vm_id}' then "
            f"'celesto sandbox create --name {vm.vm_id}' to recreate it."
        )
...
def _run_exec(args: SimpleNamespace) -> int:
    ...
    try:
        ...
        if args.start:
            ...
            recovery = (
                f"Run 'celesto sandbox start {args.vm_id}' first, or add --start to "
...
def _run_logs(args: SimpleNamespace) -> int:
    ...
    try:
        ...
        if not log_path.exists():
            recovery = f"Start it with 'celesto sandbox start {args.vm_id}' to generate logs."
        ...
        if json_output:
            if args.follow:
                return emit_error(
                    ...
                    f"'celesto sandbox logs {args.vm_id} --json' without --follow.",
                    recovery=f"Run 'celesto sandbox logs {args.vm_id} --json' without --follow.",
                )
            try:
                emit_json(
                    command_name,
...
def _run_port_expose(args: SimpleNamespace) -> int:
    """Handle ``celesto sandbox port expose``."""
    from celesto.facade import Celesto
    ...
    try:
        ...
        return _emit_cli_error(
            ...
            ValueError(
                ...
                f"Run 'celesto sandbox port expose {args.vm_id} 8080:3000'"
                f" to forward host port 8080 to sandbox port 3000, "
                f"or 'celesto sandbox port expose {args.vm_id} 3000' "
    ...
    try:
        ...
        if json_output:
            ...
            console.print(
                f"Stop with: [bold]celesto sandbox port close "
...
def _run_port_close(args: SimpleNamespace) -> int:
    """Handle ``celesto sandbox port close``."""
    from celesto.facade import Celesto
    ...
    try:
        ...
        if host_port is None:
            raise ValueError(
                ...
                f"Run 'celesto sandbox port list {args.vm_id}' to see active forwards."
            )
    except ValueError as exc:
        return _emit_cli_error(command_name, 1, exc, json_output=json_output)
...
def _start_openclaw_gateway(vm: FacadeVM, vm_id: str) -> None:
    ...
    if not installed.ok:
        raise RuntimeError(
            ...
            f"'celesto sandbox snapshot create {vm_id} --snapshot-id "
            f"{vm_id}-before-openclaw-upgrade', then 'celesto sandbox delete {vm_id}' and "
            f"'celesto openclaw start --name {vm_id} --no-attach'."
        )
    ...
    if version_match is None or version_match.group(0) != OPENCLAW_VERSION:
        ...
        raise RuntimeError(
            ...
            f"OpenClaw {OPENCLAW_VERSION}. Before replacing it, run 'celesto sandbox "
            f"snapshot create {vm_id} --snapshot-id {vm_id}-before-openclaw-upgrade', then "
            f"'celesto sandbox delete {vm_id}' and 'celesto openclaw start --name {vm_id} "
            "--no-attach'."
        )
...
"""
    ...
    if not started.ok:
        raise RuntimeError(
            ...
            f"'celesto sandbox shell {vm_id}', then "
    ...
    try:
        ...
        if not dashboard.ok:
            raise RuntimeError(
                ...
                f"run 'celesto sandbox shell {args.vm_id}', then 'openclaw dashboard'."
            )
        try:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"'{args.vm_id}'; run 'celesto sandbox shell {args.vm_id}', then "
                "'openclaw dashboard'."
            ) from exc
        ...
        close_command = (
            f"celesto sandbox port close {args.vm_id} {host_port}:{OPENCLAW_DASHBOARD_PORT}"
        )
        ...
        if isinstance(exc, VMNotFoundError):
            exc = RuntimeError(
                f"Sandbox '{args.vm_id}' was not found; run 'celesto sandbox list --all' "
                f"to choose one, or 'celesto openclaw start --name {args.vm_id} "
...
def _run_port(args: SimpleNamespace) -> int:
    """Dispatch ``celesto sandbox port <action>``."""
    ...
    render_error(
        "Usage: celesto sandbox port {expose,close,list} ... "
        "Run 'celesto sandbox port --help' for usage."
    )
    return 2
...
def _render_ui_startup(
    ...
    *,
    ...
    lines = [
        f"Starting Celesto UI on http://{host}:{port} ...",
    ...
    console_stdout().print(Panel.fit("\n".join(lines), title="Celesto UI", border_style="cyan"))
...
def _run_server_start(
    ...
    *,
    ...
    """Start the Celesto HTTP API server (backs the TypeScript/other SDKs)."""
    try:
    ...
    if port < 1 or port > 65535:
        return _emit_cli_error(
            ...
            ValueError(
                ...
                f"e.g. 'celesto server start --host {host} --port 8000'."
            ),
            json_output=False,
        )

    display_host = "localhost" if host in {"0.0.0.0", "::"} else host
    print(f"Celesto HTTP API listening on http://{display_host}:{port}")
    print(f"OpenAPI spec: http://{display_host}:{port}/openapi.json")

    try:
        uvicorn.run("celesto.server.app:create_app", host=host, port=port, factory=True)
        return 0
    except KeyboardInterrupt:
        return 130
    except Exception as exc:
...
def _run_ui(host: str, port: int, allow_beta: bool) -> int:
    ...
    try:
        uvicorn.run("celesto.dashboard.server:app", host=host, port=port)
        return 0
    except KeyboardInterrupt:
        return 130
    except Exception as exc:
...
def _render_browser_list(
    rows: list[BrowserRow],
    *,
    title: str = "Celesto Browser Sandboxes",
...
def _render_computer_list(rows: list[ComputerRow]) -> None:
    ...
    table = Table(title="Celesto Computers")
...
def _require_display_session_mode(state: Any, session_id: str, mode: str) -> None:
    ...
    if actual != mode:
        ...
        raise ValueError(
            f"'{session_id}' is not a {noun}; run 'celesto {command} list' to choose one."
        )


def _run_windows(args: SimpleNamespace) -> int:
    """Handle ``celesto windows`` commands."""
    ...
    print(
        "celesto windows: choose a subcommand (e.g. `celesto windows build-image --help`).",
        file=sys.stderr,
    )
    return 2
...
def _run_windows_build_image(args: SimpleNamespace) -> int:
    """Handle ``celesto windows build-image``."""
    from celesto.windows import WindowsImageBuilder
    ...
    if args.json:
        ...
        console.print(
            Panel.fit(
                ...
                f'  [bold]Celesto(os="windows", image="{output}", '
...
def _run_browser(args: SimpleNamespace) -> int:
    """Handle ``celesto browser`` commands."""
    ...
    if args.browser_action is None:
        render_error("Usage: celesto browser {start,stop,list,open,logs} ...")
        return 2
    ...
    if args.browser_action == "stop":
        if args.all:
            ...
            if not sessions:
                render_empty("Celesto Browser Sandboxes", "No browser sandboxes found.")
                return 0
            ...
            if failures:
                ...
                commands = ", then ".join(
                    f"`celesto browser stop {session_id}`" for session_id in failed_ids
                )
                render_error(
        ...
        try:
            ...
            if session._session_config.mode == "computer":
                raise ValueError(
                    ...
                    f"'celesto computer delete {args.session_id}' instead."
                )
            session.stop()
            ...
            if isinstance(exc, ValueError):
                ...
                reported_error = RuntimeError(
                    ...
                    f"'celesto browser stop {args.session_id}' to try again."
                )
    ...
    if args.browser_action == "open":
        ...
        try:
            ...
            if session._session_config.mode == "computer":
                raise ValueError(
                    f"'{args.session_id}' is a computer; run "
                    f"'celesto computer open {args.session_id}' instead."
                )
    ...
    if args.browser_action == "logs":
        ...
        try:
            ...
            if session._session_config.mode == "computer":
                raise ValueError(
                    f"'{args.session_id}' is a computer; run "
                    f"'celesto computer logs {args.session_id}' instead."
                )
...
def _run_computer(args: SimpleNamespace) -> int:
    ...
    if action == "templates":
        ...
        if json_output:
            ...
            table = Table(title="Celesto Computer Templates")
    ...
    if action == "list":
        try:
            ...
            if json_output:
                ...
                render_empty("Celesto Computers", "No computers found.")
            else:
    ...
    try:
        ...
        if action in {"terminal", "exec"}:
            ...
            render_error("Usage: celesto computer {start,delete,list,open,logs,templates} ...")
            return 2
        return 0
    except Exception as exc:
        return _emit_cli_error(command_name, 1, exc, json_output=json_output)
...
def build_cli() -> click.Group:
    ...
    from celesto.cli.commands import build_cli as _build_cli
