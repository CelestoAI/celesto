...
"""Celesto image-cache management — pull, list, inspect, build, and remove.

Backs the ``celesto image`` command group. Images normally download lazily
...
:func:`celesto.images.manager.resolve_image_dir`.
"""
...
from celesto.cli.prune import _VERSION_PATTERN, _format_bytes, _size_on_disk, _total_size
from celesto.images.manager import resolve_image_dir
...
def _canonical_preset(name: str) -> str:
    ...
    """
    from celesto.presets import canonical_preset_name
    ...
    if manifest.is_file():
        try:
            ...
            if isinstance(payload, dict):
                ...
                from celesto.host.lume import LUME_VERSION
...
def _pull_retry_command(
    ...
    parts = ["celesto", "image", "pull", preset]
    if arch:
        parts += ["--arch", arch]
    if vmm:
        parts += ["--vmm", vmm]
...
def _build_retry_command(
    ...
    parts = ["celesto", "image", "build", "-t", tag]
    if dockerfile:
...
def _execute_pull(
    *,
    ...
    """
    from celesto.images.published import Arch, Os, Preset, Vmm, cache_name, ensure_published_image
...
def run_image_pull(
    *,
    ...
    """Execute ``celesto image pull``."""
    from celesto.cli.main import _host_arch_for_published, _vmm_for_host
    from celesto.exceptions import ImageError
    from celesto.images.published import MANIFEST, is_preset_published
    ...
    try:
        ...
        return _fail(
            command_name,
            "Celesto doesn't publish prebuilt images for this machine. Pass the "
            f"platform explicitly, e.g. 'celesto image pull {preset} --arch amd64 "
            "--vmm firecracker'.",
    ...
    if not is_preset_published(canonical, resolved_arch, resolved_vmm, resolved_os):  # type: ignore[arg-type]
        ...
        return _fail(
            ...
            f"'celesto image pull {preset}' to use the defaults for this machine.",
            json_output=json_output,
            code="invalid_input",
            exit_code=2,
        )
...
def run_image_pull_all(
    *,
    ...
    """Execute ``celesto image pull --all``."""
    from celesto.cli.main import _host_arch_for_published, _vmm_for_host
    from celesto.images.published import Arch, Vmm, published_targets
    ...
    try:
        ...
        return _fail(
            command_name,
            "Celesto doesn't publish prebuilt images for this machine. Pass the "
            "platform explicitly, e.g. 'celesto image pull --all --arch amd64 "
            "--vmm firecracker'.",
            json_output=json_output,
            code="invalid_input",
            exit_code=2,
    ...
    *,
    ...
    try:
        ...
        return _fail(
            ...
            json_output=json_output,
            recovery="Fix the folder's permissions and run 'celesto image list' again.",
        )
    ...
    if not rows:
        render_empty(
            "Images",
            f"No cached images in {root}. Run 'celesto image pull codex' to download one.",
        )
        return 0
...
def _inspect_entry(path: Path, root: Path) -> ImageInspectEntry:
    ...
    if row["kind"] == "image" and row["current"]:
        from celesto.exceptions import ImageError
        from celesto.images.published import IMAGES_RELEASE_TAG, lookup
    ...
    if row["kind"] == "macos":
        try:
            from celesto.macos.images import MacOSImageManifest
    ...
    *,
    ...
    if not _valid_entry_name(requested):
        return _fail(
            ...
            json_output=json_output,
            code="invalid_input",
            recovery="Run 'celesto image list' to see cached images.",
            exit_code=2,
        )
    ...
    try:
        ...
        return _fail(
            ...
            f"'celesto image inspect {requested}'.",
        )
...
def run_image_rm(
    *,
    ...
    permission_recovery = (
        f"Fix the folder's permissions, then retry 'celesto image rm {requested}'."
    )
    try:
    ...
    *,
    ...
    if ipsw != "latest":
        ...
        if not ipsw_path.is_file():
            return _fail(
                ...
                json_output=json_output,
                code="invalid_input",
                recovery=f"Restore the file, or run 'celesto image build --os macos "
                f"--ipsw latest -t {tag}'.",
                exit_code=2,
            )
    ...
    try:
        ...
        return _fail(
            ...
            str(exc),
            json_output=json_output,
            recovery="Run 'celesto setup --macos', then 'celesto doctor --backend vz'.",
        )
    ...
    try:
        ...
        console_stdout().print(
            ...
            f"'celesto sandbox create --os macos --name test-mac'."
        )
        return 0
    except Exception as exc:
        return _fail(
            ...
            json_output=json_output,
            recovery=(
                "Run 'celesto image build --os macos "
...
def run_image_build(
    *,
    ...
    """Execute ``celesto image build``."""
    from celesto.exceptions import ImageError
    from celesto.images.boot import DirectKernelBoot
    from celesto.images.builder import DockerContextValue, DockerRootfsBuilder
    ...
    if not context_dir.is_dir():
        return _fail(
            ...
            f"(use '.' for the current one), e.g. 'celesto image build -t {tag} .'.",
            exit_code=2,
        )
    ...
    console.print(
        Panel.fit(
            ...
            "  [bold]from celesto import BootImage, DirectKernelBoot, Celesto\n"
            ...
            "  with Celesto.from_image(image) as vm:\n"
            "      vm.start()[/bold]\n\n"
            f"Manage it with 'celesto image list' and 'celesto image rm {_CUSTOM_DIR}/{tag}'.",
