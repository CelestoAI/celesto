...
"""Image building utilities for Celesto.
...
from celesto.images.boot import BootImage, DirectKernelBoot
from celesto.images.manager import resolve_image_dir
from celesto.kernels import KernelArch, ensure_base_kernel_for_backend
from celesto.presets.openclaw import OPENCLAW_NODE_VERSION, OPENCLAW_VERSION
from celesto.runtime.backends import resolve_backend
from celesto.runtime.boot_profiles import (
    KernelBootProfile,
    normalize_arch,
    to_published_arch,
)
from celesto.utils import RUNTIME_PRIVILEGE_SETUP_HINT, run_command
...
def _guest_agent_target_triple(arch: str | None = None) -> str:
    """Return the static Linux Rust target used inside Celesto rootfs images."""
    ...
    try:
        ...
        if actual_sha256 != expected_sha256:
            raise ImageError(
                ...
                f"run `rm -f {quoted_binary_path}` and retry so Celesto downloads it again."
            )
...
def _docker_platform_for_arch(arch: str) -> str:
    """Return Docker's platform string for a Celesto arch key."""
    if arch == "amd64":
        return "linux/amd64"
    if arch == "arm64":
        return "linux/arm64"
...
class ImageBuilder:
    ...
    Example usage::
        ...
        from celesto import ImageBuilder, Celesto, VMConfig
        from celesto.images.builder import SSH_BOOT_ARGS
    ...
    def docker_requirement_error(self) -> ImageError:
        ...
        start_hint = (
            "Docker is installed, but Celesto could not reach the Docker daemon. "
...
"""
    ...
    def _openclaw_init_script(self) -> str:
        ...
        the shared Celesto init behavior and an identifiable hostname.
        """
    ...
    def _host_arch_key() -> str:
        """Normalize host architecture to Celesto kernel key."""
    ...
    def _kernel_url_for_host(self) -> str:
        """Return the Celesto-built base kernel URL for the current host arch.
    ...
    def qemu_kernel_url_for_host(self) -> str:
        """Return the Celesto-built Image-format base kernel URL.
    ...
    def _download_kernel(self, url: str, dest: Path) -> None:
        ...
        For Celesto's published base kernel (the default path) we route through
        :func:`celesto.images.published.ensure_base_kernel`, which downloads to
        ...
        """
        from celesto.images.published import BASE_KERNELS, ensure_base_kernel
...
class DockerRootfsBuilder:
    ...
    This generic builder does not inject Celesto's guest agent. The caller owns
