...
Mirror of :mod:`celesto.env` for Windows. Uses PowerShell's
...
def _validate_windows_env_key(key: str) -> None:
    ...
    if key == _MANAGED_KEYS_SENTINEL:
        raise ValueError(
            f"{_MANAGED_KEYS_SENTINEL!r} is reserved for Celesto's internal "
...
def _read_managed_keys(ssh: SSHClient) -> list[str]:
    """Read the Celesto-managed-keys sentinel from the guest registry."""
    out = _run_ps(
        ssh,
...
def _write_managed_keys(ssh: SSHClient, keys: list[str]) -> None:
    """Persist the Celesto-managed-keys sentinel in the guest registry."""
...
def inject_env_vars(
    ...
    *,
    ...
    When *merge* is ``True`` (the default), existing Celesto-managed
    ...
    *merge* is ``False``, every previously-managed var is cleared first
    ...
    Args:
        ...
        merge: If True (default), merge with existing Celesto-managed
    ...
    Raises:
        CelestoError: If any of the PowerShell commands fail.
