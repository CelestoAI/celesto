...
- Pass the guest-visible endpoint to `celesto-browser-session`. Confirm the running Chromium process has both `--proxy-server=<endpoint>` and `--proxy-bypass-list=<-loopback>`.
...
1. Upload one content-addressed candidate per architecture. Make the Firecracker and QEMU jobs download that exact candidate rather than consulting the published catalog.
1. Gate pull requests on Linux `amd64` Firecracker and QEMU browser smokes. Gate the image release on native `amd64` and `arm64` builds and real-VM smokes for both backends.
1. In each smoke, start a browser with the proxy endpoint, inspect the Chromium command line, and run the complete attack matrix from section 2.
1. Run the existing browser/computer command, file, live-view, stop/start, and cleanup smokes with no proxy argument to prove backward compatibility.
...
1. Add browser and Linux computer entries to the published manifest, then copy every rootfs URL, size, and SHA-256 from the build output into `src/celesto/images/published.py`.
1. Copy the standalone `celesto-guest-agent-linux-<arch>.sha256` values into `src/celesto/images/builder.py::_GUEST_AGENT_RELEASE_SHA256`. These pins are separate from the rootfs manifest.
1. Run `.github/workflows/smoke-published-images.yml` against the draft tag and make it smoke the new browser/computer assets on Firecracker and QEMU for `amd64` and `arm64`.
1. Verify the source-checkout path with `uv run celesto ...`, which may build or use a local guest-agent binary.
1. Build and install the candidate wheel in a clean environment, then verify `celesto ...` downloads the standalone guest-agent asset and the pinned rootfs.
1. Publish the draft image release only after its manifest and both SHA pin sets match the tested bytes. Do not tag the Python package before these checks pass.
