...
1. **Apple Silicon only.** Do not support Intel Macs, Linux hosts, cross-architecture emulation, or non-Apple hardware.
1. **Apple Virtualization framework.** Add a `vz` backend. Do not attempt to boot macOS through the existing QEMU backend.
1. **Lume-backed first implementation.** Put Lume behind a Celesto-owned driver interface and pin a tested MIT-licensed release. This gets an end-to-end preview working without making Lume's CLI or storage model part of Celesto's public API. A native `celesto-vz` helper can replace it later without changing the SDK or CLI.
1. **Local images only.** Download a compatible IPSW from Apple and install it locally. Do not publish, push, save, or load preinstalled macOS images in the preview.
1. **Clean clone per sandbox.** Build one reusable base machine and create an APFS copy-on-write clone for each sandbox.
1. **Desktop is first-class.** Every macOS sandbox starts with a loopback-only VNC display. `celesto sandbox desktop` opens that display with the host's VNC client. Closing the viewer does not stop the sandbox.
1. **Ordinary macOS security remains enabled.** Do not disable SIP, Gatekeeper, notarization checks, or normal permission prompts.
1. **Shares are general and safe by default.** Map existing `--mount` inputs to VirtioFS. Keep host directories read-only unless `--writable-mounts` is explicit.
1. **Guest commands are deferred.** Shell, exec, file upload/download, and port tunnels remain unavailable until Celesto can provision secure per-sandbox credentials. Desktop readiness never depends on SSH.
1. **No false feature parity.** Reject unsupported snapshots, restricted internet settings, bridge networking, and vsock with short recovery messages.
1. **Maximum two running macOS guests.** Enforce the limit among Celesto-managed macOS sandboxes and handle the framework's limit cleanly if another application already consumes a slot.
...
After installation, Celesto clones the base, starts the sandbox, and reports:
...
Finder shows the share under a stable `Celesto Shared` location. Shell, exec, and file-transfer commands are deferred until secure guest credential provisioning is available.
...
Keep every Lume command, output parser, and artifact-layout assumption inside `src/celesto/macos/lume.py`. The rest of Celesto speaks only Celesto-owned request/result models.
...
Each base image under `~/.celesto/images/macos/<name>/` has a versioned `manifest.json` containing:
...
```text
~/.celesto/macos-vms/<vm-id>/
```

A clone must receive a unique machine identifier and MAC address. The spike must prove that the selected Lume clone operation does this correctly; otherwise Celesto must regenerate them before the implementation proceeds.
...
Shell, exec, file transfer, and port tunnels are deferred until Celesto can provision secure per-sandbox credentials. The preview does not expose Lume's fixed guest account through those public operations.
...
- Install the user's Celesto public key.
...
If secure randomized credentials cannot be achieved with the pinned Lume release, the project must either patch the MIT-licensed helper or move the necessary provisioning code into `celesto-vz` before enabling guest commands.
...
- \[ \] Decide during the spike whether `celesto setup --backend vz` downloads a pinned standalone binary or provides a pinned Homebrew command. Prefer a Celesto-managed checksum-verified binary under `~/.celesto/bin` if upstream packaging supports it.
...
**Exit criteria:** image build is resumable or safely restartable, inspection is truthful about disk use, and no macOS image leaves the local machine through Celesto commands.
...
- \[ \] Implement `VzRuntimeAdapter` and register it in `CelestoManager._runtime_adapter_for_backend()`.
...
- \[ \] Count running Celesto macOS guests before launch and return a clear two-guest-limit message.
...
- \[ \] Add `Celesto.open_desktop()` and `Celesto.desktop_endpoint`; do not change the existing `Celesto.desktop()` class factory used for Linux desktop sessions.
...
- \[ \] Make shares visible in Finder under a stable `Celesto Shared` naming convention.
...
| APFS clone semantics unavailable on external storage | Preflight filesystem type and fail with a command using the default Celesto data directory; consider full-copy fallback only later. |
| User expects Firecracker-like boot density | Document full-macOS resource and startup characteristics before download. |
| Apple enforces two active guests across applications | Enforce Celesto's own count and translate framework errors when external VMs use a slot. |
...
1. Replace Lume with a signed/notarized `celesto-vz` helper if upstream coupling or credential provisioning remains problematic.
1. Add host-side noVNC for a browser viewer and richer dashboard integration.
1. Add screenshots and generic keyboard/mouse automation as a separate computer-use API.
1. Add disk-only stopped snapshots after defining multi-artifact atomicity and restore semantics.
1. Evaluate Apple saved-state suspend/resume separately from durable snapshots.
1. Evaluate multiple named macOS base images and version pinning.
1. Evaluate safe writable-share workflows and explicit copy-in/copy-out UX.
1. Consider hosted macOS only as a separate legal and infrastructure project.

______________________________________________________________________

## Definition of done

The implementation is complete when a developer with a supported Apple Silicon Mac can prepare a local macOS image, create an isolated sandbox clone, open and reconnect to its desktop, access an explicitly shared folder in Finder, install and run an application through ordinary macOS interactions, stop/restart the sandbox, delete it, and create a clean replacement—without DMG-specific product behavior, unintended writes to the host, exposed desktop ports, leaked credentials, or regressions to existing Celesto backends.
